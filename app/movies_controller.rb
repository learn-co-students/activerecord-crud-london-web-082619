...
def can_find_by_multiple_attributes
  ...
  # release_date == 2000
  # director == "Me"
  Movie.all.find {|movie| movie.title && movie.release_date && movie.director}
end

def can_find_using_where_clause_and_be_sorted
  # For this test return all movies released after 2002 and ordered by 
  # release date descending
  Movie.all.where("release_date > 2002").sort{|a, b| b <=> a}
end

def can_be_found_updated_and_saved
  # Updtate the title "Awesome Flick" to "Even Awesomer Flick", save it, then return it
  Movie.create(title: "Awesome Flick")
  movie = Movie.all.find {|movie| movie.title == "Awesome Flick"}
...
def can_update_using_update_method
  ...
  movie = Movie.all.find {|movie| movie.title == "Wat?"}
